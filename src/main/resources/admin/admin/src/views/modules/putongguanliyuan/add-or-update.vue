<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="管理员账号" prop="guanliyuanzhanghao">
          <el-input v-model="ruleForm.guanliyuanzhanghao" 
              placeholder="管理员账号" clearable  :readonly="ro.guanliyuanzhanghao"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="管理员账号" prop="guanliyuanzhanghao">
              <el-input v-model="ruleForm.guanliyuanzhanghao" 
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.gerenzhaopian" label="个人照片" prop="gerenzhaopian">
          <file-upload
          tip="点击上传个人照片"
          action="file/upload"
          :limit="3"
          ...
          @change="gerenzhaopianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.gerenzhaopian" label="个人照片" prop="gerenzhaopian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.gerenzhaopian.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.lianxidianhua = true;
          ...
          if(o=='gerenzhaopian'){
            this.ruleForm.gerenzhaopian = obj[o];
	    this.ro.gerenzhaopian = true;
            continue;
          }
        }
      }
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `putongguanliyuan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.putongguanliyuanCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    gerenzhaopianUploadChange(fileUrls) {
