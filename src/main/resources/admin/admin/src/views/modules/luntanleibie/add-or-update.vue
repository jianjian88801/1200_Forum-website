<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="论坛类别" prop="luntanleibie">
          <el-input v-model="ruleForm.luntanleibie" 
              placeholder="论坛类别" clearable  :readonly="ro.luntanleibie"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="论坛类别" prop="luntanleibie">
              <el-input v-model="ruleForm.luntanleibie" 
                placeholder="论坛类别" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='luntanleibie'){
            this.ruleForm.luntanleibie = obj[o];
	    this.ro.luntanleibie = true;
            continue;
          }
        }
      }
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `luntanleibie/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.luntanleibieCrossAddOrUpdateFlag = false;
                  this.parent.search();
